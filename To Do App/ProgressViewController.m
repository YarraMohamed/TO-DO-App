...
@property (weak, nonatomic) IBOutlet UIImageView *progressPhoto;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    _inProgress = [NSMutableArray new];
    _seprated = NO;
    ...
    if(self.inProgress.count == 0){
        [self.tableView setHidden:YES];
        [self.progressPhoto setHidden:NO];
    }else{
        [self.tableView setHidden:NO];
        [self.progressPhoto setHidden:YES];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Task *task = [Task new];
    if ([self seprated] == NO) {
        task = _inProgress[indexPath.row];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"Yes" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            ...
            if (!self.seprated) {
                taskToDelete = self.inProgress[indexPath.row];
                [self.inProgress removeObjectAtIndex:indexPath.row];
            } else {
                ...
                [self.inProgress removeObject:taskToDelete];
            }
